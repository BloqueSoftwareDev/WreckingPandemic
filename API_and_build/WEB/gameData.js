...
function connectToDB()
{
    ...
    try{
        return mysql.createConnection({host:'127.0.0.1', user:'root', password:'Mariobros101', database:'wrecking_pandemic'});
...
// This api gets the data from the database
app.get('/api/gamedata', (request, response) => {
    let connection = connectToDB();
    connection.query('SELECT * FROM videogame', (err,rows) => {
      if(err) throw err;
      response.json(rows);
  })})
...
app.post('/api/gamedata', (request, response)=>{
    ...
    try{
        ...
        const query = connection.query('insert into videogame set ?', request.body ,(error, results, fields)=>{
